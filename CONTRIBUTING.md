...
To start the Coco CLI from the source code (after building), run the following
...
If you'd like to run the source build outside of the project folder, you can
utilize `npm link path/to/coco/packages/cli` (see:
[docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) or
`alias coco="node path/to/coco/packages/cli"` to run with `coco`
